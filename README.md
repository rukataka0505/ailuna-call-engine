...
本サーバーでは、**全体を 8kHz / mono で統一**しています。
...
* サーバー内部：
  ...
  * 必要に応じて μ-law 8kHz ⇔ PCM16 8kHz に変換
  * サンプリングレートの変換（8kHz → 16kHz など）は行いません
* Realtime API とのやり取り：

  * 8kHz 前提で音声を送受信する設計

この方針により、処理のシンプルさとレイテンシの低さを優先しています。
将来、16kHz 等に変更したい場合は「どこでサンプリングレートを変更するか」を
このパイプライン設計に沿って調整してください。
